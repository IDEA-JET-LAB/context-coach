...
#### CRITICAL: API Key Conflict Resolution

**PROBLEM:** If both `GOOGLE_API_KEY` and `GEMINI_API_KEY` are set, the google-genai library may use `GOOGLE_API_KEY` by default, which can cause "Image generation is not available in your country" errors or other unexpected behavior.

**SOLUTION:** When running Python generation scripts, ALWAYS unset `GOOGLE_API_KEY` first:

```bash
# In shell commands:
unset GOOGLE_API_KEY && python3 script.py

# Or explicitly use GEMINI_API_KEY in Python:
API_KEY = os.environ.get("GEMINI_API_KEY")
client = genai.Client(api_key=API_KEY)  # Pass explicitly, don't rely on auto-detection
```

**In Python scripts, ALWAYS:**
1. Use `os.environ.get("GEMINI_API_KEY")` explicitly
2. Pass the key to `genai.Client(api_key=API_KEY)` directly
3. Do NOT rely on automatic key detection from the library

**Example correct Python setup:**
```python
import os
from google import genai

# CORRECT: Explicitly get GEMINI_API_KEY
API_KEY = os.environ.get("GEMINI_API_KEY")
if not API_KEY:
    print("ERROR: GEMINI_API_KEY not set")
    sys.exit(1)

# CORRECT: Pass key explicitly to client
...
1. Check if `GEMINI_API_KEY` environment variable exists
2. If both GOOGLE_API_KEY and GEMINI_API_KEY exist, prefer GEMINI_API_KEY
3. In generation scripts, explicitly pass the key to avoid conflicts
4. If truly missing, then guide user:
   ```bash
   # Add to ~/.zshrc
