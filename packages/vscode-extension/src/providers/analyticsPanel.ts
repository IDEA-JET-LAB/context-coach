...
<body>
  ...
  /**
   * Install both prompt capture (UserPromptSubmit) and response capture (Stop) hooks.
   * IMPORTANT: This must be kept in sync with packages/cli/src/lib/hooks.ts setupClaudeHooks()
  ...
  private async installCaptureHook(workspacePath: string): Promise<void> {
    ...
    // Configure UserPromptSubmit hook (prompt capture)
    const promptHookCommand = `bash "$CLAUDE_PROJECT_DIR"/.claude/hooks/contextor-capture.sh`;
    const promptHookEntry: HookEntry = {
      matcher: ".*",
      hooks: [{ type: "command", command: promptHookCommand, timeout: 5000 }],
    };

    settings.hooks ??= {};
    const existingPromptHooks = settings.hooks.UserPromptSubmit ?? [];

    // Check if Contextor prompt hook already exists
    const promptIdx = existingPromptHooks.findIndex((entry) =>
      entry.hooks?.some((h) => h.command.includes("contextor-capture"))
    );

    if (promptIdx >= 0) {
      existingPromptHooks[promptIdx] = promptHookEntry;
    } else {
      existingPromptHooks.push(promptHookEntry);
    }

    settings.hooks.UserPromptSubmit = existingPromptHooks;

    // Configure Stop hook (response capture)
    // NOTE: Stop hooks do NOT support the matcher field - they are global events
    const responseHookCommand = `bash "$CLAUDE_PROJECT_DIR"/.claude/hooks/contextor-response.sh`;
    const responseHookEntry: HookEntry = {
      hooks: [{ type: "command", command: responseHookCommand, timeout: 5000 }],
    };

    const existingStopHooks = settings.hooks.Stop ?? [];

    // Check if Contextor response hook already exists
    const stopIdx = existingStopHooks.findIndex((entry) =>
      entry.hooks?.some((h) => h.command.includes("contextor-response"))
    );

    if (stopIdx >= 0) {
      existingStopHooks[stopIdx] = responseHookEntry;
    } else {
      existingStopHooks.push(responseHookEntry);
    }
    ...
    // Write prompt capture script
    const captureScript = this.getCaptureScriptContent();
    const captureScriptPath = vscode.Uri.joinPath(hooksDir, "contextor-capture.sh");
    await vscode.workspace.fs.writeFile(captureScriptPath, Buffer.from(captureScript, "utf-8"));

    // Write response capture script
    const responseScript = this.getResponseScriptContent();
    const responseScriptPath = vscode.Uri.joinPath(hooksDir, "contextor-response.sh");
    await vscode.workspace.fs.writeFile(responseScriptPath, Buffer.from(responseScript, "utf-8"));
    ...
    try {
      ...
      await execAsync(`chmod +x "${captureScriptPath.fsPath}"`);
      await execAsync(`chmod +x "${responseScriptPath.fsPath}"`);
    } catch {
...
`;
   ...
   * IMPORTANT: This must be kept in sync with packages/cli/src/lib/hooks.ts getResponseScriptContent()
   * See CLAUDE.md "Hook Script Sync Requirements" section for maintenance instructions.
   */
  private getResponseScriptContent(): string {
    return `#!/bin/bash
